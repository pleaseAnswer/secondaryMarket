...
async function connect() {
    ...
    const db = client.db(dbName); //phone
    return {
        client,
        db
    }
 ...
 * @param {object} arrName                数组名
 * db.class.update({name: "class1"}, {$addToSet:{student:{name: "zhaoliu", age: 17}}});
 */
async function arrcreate(colName, query, data) {
    const { db, client } = await connect();
    let collection = db.collection(colName);
    ...
    let result = await collection.update(query,{$addToSet:{list:data}});
